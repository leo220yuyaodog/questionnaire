<template>
  <el-row type="flex" justify="center" align="middle" class="header-row">
    ...
    <el-col :span="3">
      <el-avatar v-if="!$store.state.login" style="cursor: pointer" @click.native="gotoLogin">登录</el-avatar>
      <el-dropdown v-if="$store.state.login" class="user-profile">
        <el-avatar
          ><b>{{ $store.state.username.substr(0, 1) }}</b></el-avatar
        >
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item
            class="share-copy-link"
            icon="el-icon-share"
            :data-clipboard-text="servername"
