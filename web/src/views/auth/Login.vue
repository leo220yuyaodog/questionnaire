<template>
  <div>
    ...
    >
      ...
      <el-link type="primary" class="link" @click="registerChoose = true"> 注册账户 </el-link>
    ...
    >
      ...
      <el-link type="primary" class="link" @click="registerChoose = false"> 登录账户 </el-link>
  ...
  methods: {
    ...
    submitLogin() {
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          this.axios
            .post(`/api/login?username=${this.loginForm.username}&password=${this.loginForm.password}`)
            .then((res) => {
              this.loading = false;
              if (res.data === 'wrong') {
    ...
    submitRegister() {
      this.$refs.registerForm.validate((valid) => {
        if (valid) {
          this.axios
            .post(`/api/register?username=${this.registerForm.username}&password=${this.registerForm.password}`)
            .then((res) => {
              this.loading = false;
