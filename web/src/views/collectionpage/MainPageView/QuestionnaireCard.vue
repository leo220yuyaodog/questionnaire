<template>
  <div>
    ...
    <el-dialog v-model:visible="dialogOutlineDataVisible" :title="title">
      <div>
        问卷链接为：
        <el-link
          class="copy-link"
          target="_blank"
          :data-clipboard-text="servername + '/fillin/' + id"
      ...
      <div class="description-box">
        <div><b>问卷描述：</b>{{ description }}</div>
        <div><b>问卷状态：</b>{{ translateLabel[status] }}</div>
        <div><b>填写人数：</b>{{ fillCount }}</div>
        <div><b>发布时间：</b>{{ createTime }}</div>
        <div><b>截止时间：</b>{{ endTime }}</div>
      </div>

      <el-dialog v-model:visible="deleteVisible" width="30%" title="确认删除？" append-to-body center>
      ...
      <el-button v-if="status === 'collecting' || status === 'closed'" icon="el-icon-search" @click="gotoFillIn"
        >预 览
      </el-button>
...
export default {
  ...
  methods: {
    ...
    closeForm() {
      this.axios
        .post(`/api/closeQuestionnaire?questionnaireId=${this.id}`)
        .then(() => {
